...
class TeamNode(object):
    ...
    def __init__(self , teamName , lostTo , beat , points):
...
def run():
    ...
    if containsChains[0]:
        ...
        #RESULTRANKS AS THE FINAL RESULT. (last is best)
...
def pointSort(teamNodes):
    ...
    while not dropOut:
        ...
        if len(lostTo) == 0:
            dropOut = True
        elif random.randint(1 , 1000) > (winLossRatio * 1000 * DROP_OUT_MODIFIER):
            dropOut = True
        else: #Traverse to a node you lost to.
            print "Traverse"
