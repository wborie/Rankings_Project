...
def topological_sort(node , rankedList):
    ...
    if node.lostTo == []:
        ...
        while node.lostTo != []: #Remove all children
            topological_sort(node.lostTo[0] , rankedList)
...
def run():
    ...
    for x in xrange(len(teamNodes)):
        topological_sort(teamNodes[x] , resultRanks)
